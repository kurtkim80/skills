...
- [ ] 测试状态（若项目有质量链）
新会话入口：（按工具——Deep Code：输入「<项目名> 接手」走 project-handoff 或 /resume；Cursor：历史面板 + 项目 rules 文件）
```
### 健康时（继续）或切换反而亏时
> 会话健康（压缩 12%）——继续。
...
ls .handoff/index 2>/dev/null && echo "交接存储在"  # 现行工作存储
ls HANDOFF.md 2>/dev/null && echo "旧模型待迁移"    # 旧交接件，迁移前不算现行
```
...
- [ ] 新会话入口指引明确（按工具：/resume / project-handoff / 历史面板 + rules）
## 新工具接入（在其他工具中使用本技能）
