...
param(
    ...
    #       a false negative, so validation fails and the script exits 1.
    #
...
function Assert-TargetHost {
    ...
    if ($resolved -ne $current) {
        Write-Warning ("Target platform is '$resolved' but this host is '$current'. A profile " +
            ...
            "This mismatch would fail validation. Generate on '$resolved'.")
    }
    ...
    if ($resolvedVersion -ne 'Current') {
        ...
        if ($hostMm -gt $targetMm) {
            ...
            Write-Warning ("Target is PowerShell $targetMm but this host is $hostMm. The profile " +
                ...
                "still fail on $targetMm. This mismatch would fail validation. Generate on $targetMm.")
        }
...
if ($result.UnpinnedModules.Count -gt 0) {
    Write-Host ''
    Write-Host "FAIL: module versions newer than the pin reached the profile:" -ForegroundColor Red
    foreach ($name in $result.UnpinnedModules) { Write-Host "    $name" -ForegroundColor Red }
    Write-Host ("These are installed outside the provision directory, so they could not be pruned. " +
        ...
        "suppressed. Remove the newer versions from the generating host's PSModulePath and regenerate " +
        "with the requested pins.") -ForegroundColor Red
}
