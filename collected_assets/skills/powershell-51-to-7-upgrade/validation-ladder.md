...
2. **Static compatibility (needs PSScriptAnalyzer) — primary gate.**
   ...
   **Only run this rung against a profile validated as `PASS` or `WARN`.** The
   generator writes a `<profile>.validation.json` next to the profile. If its
   `Validation.Status` is `FAIL`, the profile can produce false *passes* through
   contamination, module versions newer than the pin, a platform mismatch, or a
   generating host newer than the target PowerShell version. Correct every cause
   recorded in the sidecar and regenerate: use a clean session for contamination,
   remove module versions above the pin from the generating host's `PSModulePath`,
   and use the target platform and PowerShell major/minor version for mismatches.
   A clean session alone does not fix the other causes.
